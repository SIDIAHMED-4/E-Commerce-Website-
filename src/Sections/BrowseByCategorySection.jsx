...
const BrowseByCategorySection = () => {
  ...
  const CategoryCard = ({ icon, name }) => (
    <Link to="Category" className="group">
      <div
        className="flex flex-col items-center justify-center gap-4 bg-white border border-gray-200 rounded-2xl py-8 transition-all duration-300 
                 hover:bg-cyan-400 hover:-translate-y-2 hover:shadow-lg hover:invert"
      >
  ...
  return (
    ...
    <div className="flex flex-col gap-8 sm:gap-0 sm:flex-row items-center justify-between">
      <div  className="mt-8  gap-4 flex flex-col">
        <Red text={i18n.t("category.redTitle")} />
        <BigText text={i18n.t("category.title")}/>
      </div>
      <Arrow />
    </div>
      <div className="my-11">
        <Grid container spacing={2} justifyContent="center">
          {categories.map((categorie) => (
            <Grid item key={categorie.name} xs={8} sm={6} md={4} lg={2}>
              <CategoryCard icon={categorie.icon} name={categorie.name} />
            </Grid>
          ))}
        </Grid>
      </div>
